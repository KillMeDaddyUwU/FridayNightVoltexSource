...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...ClientPrefs.arrowHSV.length) {
			...
			note.frames = Paths.getSparrowAtlas('Skins/' + CoolUtil.skinTypes[skinSelected]);
			...
			ln.frames = Paths.getSparrowAtlas('Skins/' + CoolUtil.skinTypes[skinSelected]);
	...
	function changeSkin(change:Int = 0)
	{
		...
		for (i in 0...grpNotes.length)
		{
			...
			{
				item[j].frames = Paths.getSparrowAtlas('Skins/' + CoolUtil.skinTypes[skinSelected]);
				item[j].animation.addByPrefix('idle', animations[j]);
				item[j].animation.play('idle');
				grpLN.members[j].frames = Paths.getSparrowAtlas('Skins/' + CoolUtil.skinTypes[skinSelected]);
